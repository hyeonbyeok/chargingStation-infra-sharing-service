...
<link rel=stylesheet href="/css/cscfview.css">
<script>
document.title = "건의사항";
	...
	$(function(){
		...
		if(${sessionScope.member.id == 'admin'}){
			$.ajax("",{
				method:"post",
				contentType:"application/json",
				dataType : "json",
				data : JSON.stringify(item),
				success: result => console.log(`성공 , ${item.stats}`),
		...
		<div id="container">
				<div class="cont_wrap">
					<div class="mainwrap_title">
						...
						<ul class="mw_sans">
							<li class="hasiball"><span>작성자  :  ${item.id}</span></li>
						</ul>
					</div>
				...
				<div class="img_wrap">
			         <ul>
			            <c:if test="${images == null || images.size() < 1}">
			               <li>등록 된 이미지파일이 없습니다.</li>
			            </c:if>
			            <c:forEach var="image" items="${images}">
			               <li><img src="/upload/${image.uuid}_${image.filename}" class="img1"/></li>
			            </c:forEach>
			         </ul>
			     </div>
